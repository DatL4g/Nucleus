@echo off
REM Compiles nucleus_autolaunch.cpp into per-architecture DLLs (x64 + ARM64).
REM The outputs are placed in the JAR resources so they ship with the library.
REM
REM Prerequisites: Visual Studio Build Tools (MSVC) with Windows SDK 10.0.17763+.
REM Usage: build.bat

setlocal enabledelayedexpansion

set "SCRIPT_DIR=%~dp0"
set "SRC=%SCRIPT_DIR%nucleus_autolaunch.cpp"
...
cl /LD /O1 /EHsc /std:c++17 /GS- /nologo ^
    ...
    "%SRC%" ^
    /Fe:"%OUT_DIR_X64%\nucleus_autolaunch.dll" ^
    /link advapi32.lib kernel32.lib ole32.lib runtimeobject.lib wtsapi32.lib
if errorlevel 1 (
...
echo.
echo Built DLLs:
if exist "%OUT_DIR_X64%\nucleus_autolaunch.dll" echo   %OUT_DIR_X64%\nucleus_autolaunch.dll
if exist "%OUT_DIR_ARM64%\nucleus_autolaunch.dll" echo   %OUT_DIR_ARM64%\nucleus_autolaunch.dll

REM Clear NativeLibraryLoader cache (both .dll and .fingerprint)
if defined LOCALAPPDATA (
    ...
    if exist "!CACHE_BASE!\win32-x64\nucleus_autolaunch.dll.fingerprint" (
        del /q "!CACHE_BASE!\win32-x64\nucleus_autolaunch.dll.fingerprint"
    )
