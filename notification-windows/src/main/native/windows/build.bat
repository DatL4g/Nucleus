@echo off
REM Compiles nucleus_notification_windows.cpp into per-architecture DLLs (x64 + ARM64).
...
set "SCRIPT_DIR=%~dp0"
set "SRC=%SCRIPT_DIR%nucleus_notification_windows.cpp"
...
cl /LD /O1 /EHsc /std:c++17 /GS- /nologo ^
    ...
    "%SRC%" ^
    /Fe:"%OUT_DIR_X64%\nucleus_notification_windows.dll" ^
...
echo.
echo Built DLLs:
if exist "%OUT_DIR_X64%\nucleus_notification_windows.dll" echo   %OUT_DIR_X64%\nucleus_notification_windows.dll
if exist "%OUT_DIR_ARM64%\nucleus_notification_windows.dll" echo   %OUT_DIR_ARM64%\nucleus_notification_windows.dll
