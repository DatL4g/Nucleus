...
- `graalvm-runtime` auto-includes `composeResources/*`, `nucleus/native/*`, and `META-INF/services/*` via `reachability-metadata.json` resource globs (the deprecated `-H:IncludeResources` option was dropped). The app's own resources (icons, fonts, `.svg`/`.ttf`/`.otf`, …) are registered per-root by the plugin's `autoIncludeResources` (literal-prefixed globs like `icons/**`), so no blanket `**/*.ext` pattern is needed
