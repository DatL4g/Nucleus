@echo off
REM Compiles nucleus_taskbar_progress.c into per-architecture DLLs (x64 + ARM64).
...
set "SCRIPT_DIR=%~dp0"
set "SRC=%SCRIPT_DIR%nucleus_taskbar_progress.c"
...
cl /LD /O1 /GS- /nologo ^
    ...
    "%SRC%" ^
    /Fe:"%OUT_DIR_X64%\nucleus_taskbar_progress.dll" ^
    /link /NODEFAULTLIB /ENTRY:DllMain kernel32.lib ole32.lib
if errorlevel 1 (
...
echo.
echo Built DLLs:
if exist "%OUT_DIR_X64%\nucleus_taskbar_progress.dll" echo   %OUT_DIR_X64%\nucleus_taskbar_progress.dll
if exist "%OUT_DIR_ARM64%\nucleus_taskbar_progress.dll" echo   %OUT_DIR_ARM64%\nucleus_taskbar_progress.dll

REM ---- Clear NativeLibraryLoader cache so the new DLL is picked up at next run ----
set "DLL_NAME=nucleus_taskbar_progress.dll"
if defined LOCALAPPDATA (
    set "CACHE_BASE=%LOCALAPPDATA%\nucleus\native"
) else (
    set "CACHE_BASE=%USERPROFILE%\AppData\Local\nucleus\native"
)
if exist "%CACHE_BASE%\win32-x64\%DLL_NAME%" (
    del /q "%CACHE_BASE%\win32-x64\%DLL_NAME%" 2>nul
    echo Cleared cache: %CACHE_BASE%\win32-x64\%DLL_NAME%
)
if exist "%CACHE_BASE%\win32-aarch64\%DLL_NAME%" (
    del /q "%CACHE_BASE%\win32-aarch64\%DLL_NAME%" 2>nul
    echo Cleared cache: %CACHE_BASE%\win32-aarch64\%DLL_NAME%
)
