...
```proguard
-keep class dev.nucleusframework.notification.linux.NativeLinuxNotificationBridge {
    native <methods>;
    static ** on*(...);
}
```
...
[
  {
    "type": "dev.nucleusframework.notification.linux.NativeLinuxNotificationBridge",
    "methods": [
