...
Pulls in `core-runtime`, `compose.desktop.common`, and `sf-symbols` transitively. Consumers can use type-safe SF Symbol constants via `dev.nucleusframework.sfsymbols.*` without declaring `sf-symbols` explicitly.
...
[
  {
    "type": "dev.nucleusframework.menu.macos.NativeNsMenuBridge",
    "methods": [
