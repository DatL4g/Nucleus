@echo off
REM Compiles nucleus_scheduler.cpp into per-architecture DLLs (x64 + ARM64).
...
set "SCRIPT_DIR=%~dp0"
set "SRC=%SCRIPT_DIR%nucleus_scheduler.cpp"
...
cl /LD /O1 /EHsc /std:c++17 /GS- /nologo ^
    ...
    "%SRC%" ^
    /Fe:"%OUT_DIR_X64%\nucleus_scheduler.dll" ^
    /link taskschd.lib ole32.lib oleaut32.lib kernel32.lib
if errorlevel 1 (
...
echo.
echo Built DLLs:
if exist "%OUT_DIR_X64%\nucleus_scheduler.dll" echo   %OUT_DIR_X64%\nucleus_scheduler.dll
if exist "%OUT_DIR_ARM64%\nucleus_scheduler.dll" echo   %OUT_DIR_ARM64%\nucleus_scheduler.dll
