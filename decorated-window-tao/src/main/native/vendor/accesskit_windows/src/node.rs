...
impl NodeWrapper<'_> {
    ...
    // Vendored-fork: ExpandCollapse pattern support (ported from
    // accesskit_windows 0.34). Without this, nodes with expand/collapse
    // state are neither invocable nor expandable in UIA.
    ...
    fn expand_collapse_state(&self) -> ExpandCollapseState {
        match self.0.data().is_expanded() {
            ...
            // Leaf / no expand state — should not be queried (pattern off).
...
#[implement(
    ...
    ISelectionProvider,
    ITextProvider,
    // Vendored-fork: ExpandCollapse — required so expand/collapse controls
    // (not invocable per accesskit_consumer) remain activatable via UIA.
...
impl PlatformNode {
    ...
    // Vendored-fork: Expand/Collapse → Action::Expand/Collapse (mapped to
    // Compose click handlers on the JVM side for expandable groups).
...
patterns! {
    ...
    // Vendored-fork: ExpandCollapse (from accesskit_windows 0.34).
