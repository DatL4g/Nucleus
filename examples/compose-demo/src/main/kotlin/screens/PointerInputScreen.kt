...
internal fun PointerInputScreen() {
    Column(verticalArrangement = Arrangement.spacedBy(16.dp)) {
        ...
        Section(
            "detectTapGestures",
            "onTap / onDoubleTap / onLongPress / onPress reported by androidx.compose.foundation.gestures.detectTapGestures.",
        ) {
        ...
        Section(
            "Secondary / middle button",
            "awaitPointerEventScope + PointerEvent.button matching — upstream has no first-class right/middle-click gesture detector.",
        ) {
                                ...
                                while (true) {
                                    ...
                                    when (vEv.button) {
                                        PointerButton.Secondary -> {
                                            vCh.consume()
                                            vSecondary =
                                                "secondary @ (${vCh.position.x.toInt()}, ${vCh.position.y.toInt()})"
                                        }
        ...
        Section(
            "detectDragGestures",
            "onDragStart / onDrag (per-frame delta) / onDragEnd / onDragCancel from androidx.compose.foundation.gestures.detectDragGestures.",
        ) {
                            ...
                            .pointerInput(Unit) {
                                detectDragGestures(
                                    ...
                                    onDrag = { _, delta ->
                                        ...
                                        vOffsetY += delta.y
                                        vLast =
                                            "dx=${delta.x.toInt()}, dy=${delta.y.toInt()}"
                                    },
