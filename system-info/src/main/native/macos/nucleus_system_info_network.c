...
Java_dev_nucleusframework_systeminfo_macos_NativeMacOsSystemInfoBridge_nativeNetworkCount(
...
Java_dev_nucleusframework_systeminfo_macos_NativeMacOsSystemInfoBridge_nativeNetworkNames(
    JNIEnv *env, jclass clazz) {
...
JNIEXPORT jobjectArray JNICALL
Java_dev_nucleusframework_systeminfo_macos_NativeMacOsSystemInfoBridge_nativeNetworkMacAddresses(
    JNIEnv *env, jclass clazz) {
    if (g_net_count <= 0) return NULL;
    const char **arr = (const char **)malloc(g_net_count * sizeof(char *));
