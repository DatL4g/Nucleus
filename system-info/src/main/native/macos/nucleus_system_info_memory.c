...
Java_dev_nucleusframework_systeminfo_macos_NativeMacOsSystemInfoBridge_nativeTotalMemory(
...
JNIEXPORT jlong JNICALL
Java_dev_nucleusframework_systeminfo_macos_NativeMacOsSystemInfoBridge_nativeFreeMemory(
    JNIEnv *env, jclass clazz) {
...
JNIEXPORT jlong JNICALL
Java_dev_nucleusframework_systeminfo_macos_NativeMacOsSystemInfoBridge_nativeAvailableMemory(
    JNIEnv *env, jclass clazz) {
    int64_t total = 0;
    sysctl_int64("hw.memsize", &total);
    vm_statistics64_data_t stats;
...
JNIEXPORT jlong JNICALL
Java_dev_nucleusframework_systeminfo_macos_NativeMacOsSystemInfoBridge_nativeUsedMemory(
    JNIEnv *env, jclass clazz) {
    vm_statistics64_data_t stats;
    if (get_vm_stats(&stats) != 0) return 0;
    vm_size_t ps = get_page_size();
...
JNIEXPORT jlong JNICALL
Java_dev_nucleusframework_systeminfo_macos_NativeMacOsSystemInfoBridge_nativeTotalSwap(
    JNIEnv *env, jclass clazz) {
    swap_info_t si;
...
JNIEXPORT jlong JNICALL
Java_dev_nucleusframework_systeminfo_macos_NativeMacOsSystemInfoBridge_nativeFreeSwap(
    JNIEnv *env, jclass clazz) {
    swap_info_t si;
    if (get_swap_info(&si) != 0) return 0;
...
JNIEXPORT jlong JNICALL
Java_dev_nucleusframework_systeminfo_macos_NativeMacOsSystemInfoBridge_nativeUsedSwap(
    JNIEnv *env, jclass clazz) {
    swap_info_t si;
    if (get_swap_info(&si) != 0) return 0;
