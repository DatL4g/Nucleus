...
Java_dev_nucleusframework_systeminfo_linux_NativeLinuxSystemInfoBridge_nativeBatteryPresent(
    JNIEnv *env, jclass clazz) {
    return find_battery_path() != NULL ? JNI_TRUE : JNI_FALSE;
}

JNIEXPORT jboolean JNICALL
Java_dev_nucleusframework_systeminfo_linux_NativeLinuxSystemInfoBridge_nativeBatteryExternalConnected(
    JNIEnv *env, jclass clazz) {
    return is_ac_online() ? JNI_TRUE : JNI_FALSE;
}

JNIEXPORT jboolean JNICALL
Java_dev_nucleusframework_systeminfo_linux_NativeLinuxSystemInfoBridge_nativeBatteryIsCharging(
    JNIEnv *env, jclass clazz) {
...
JNIEXPORT jboolean JNICALL
Java_dev_nucleusframework_systeminfo_linux_NativeLinuxSystemInfoBridge_nativeBatteryFullyCharged(
    JNIEnv *env, jclass clazz) {
    const char *bat = find_battery_path();
    if (!bat) return JNI_FALSE;
    char path[512];
...
Java_dev_nucleusframework_systeminfo_linux_NativeLinuxSystemInfoBridge_nativeBatteryCurrentCapacity(
    JNIEnv *env, jclass clazz) {
    const char *bat = find_battery_path();
    if (!bat) return 0;
...
JNIEXPORT jint JNICALL
Java_dev_nucleusframework_systeminfo_linux_NativeLinuxSystemInfoBridge_nativeBatteryMaxCapacity(
    JNIEnv *env, jclass clazz) {
    const char *bat = find_battery_path();
    if (!bat) return 0;
...
JNIEXPORT jint JNICALL
Java_dev_nucleusframework_systeminfo_linux_NativeLinuxSystemInfoBridge_nativeBatteryDesignCapacity(
    JNIEnv *env, jclass clazz) {
    const char *bat = find_battery_path();
    if (!bat) return 0;
...
JNIEXPORT jint JNICALL
Java_dev_nucleusframework_systeminfo_linux_NativeLinuxSystemInfoBridge_nativeBatteryCycleCount(
    JNIEnv *env, jclass clazz) {
    const char *bat = find_battery_path();
    if (!bat) return 0;
...
JNIEXPORT jint JNICALL
Java_dev_nucleusframework_systeminfo_linux_NativeLinuxSystemInfoBridge_nativeBatteryVoltage(
    JNIEnv *env, jclass clazz) {
    const char *bat = find_battery_path();
    if (!bat) return 0;
...
JNIEXPORT jint JNICALL
Java_dev_nucleusframework_systeminfo_linux_NativeLinuxSystemInfoBridge_nativeBatteryAmperage(
    JNIEnv *env, jclass clazz) {
    const char *bat = find_battery_path();
    if (!bat) return 0;
...
Java_dev_nucleusframework_systeminfo_linux_NativeLinuxSystemInfoBridge_nativeBatteryTemperature(
    JNIEnv *env, jclass clazz) {
    const char *bat = find_battery_path();
...
JNIEXPORT jint JNICALL
Java_dev_nucleusframework_systeminfo_linux_NativeLinuxSystemInfoBridge_nativeBatteryTimeRemaining(
    JNIEnv *env, jclass clazz) {
    const char *bat = find_battery_path();
    if (!bat) return -1;
...
Java_dev_nucleusframework_systeminfo_linux_NativeLinuxSystemInfoBridge_nativeBatteryManufacturer(
    JNIEnv *env, jclass clazz) {
    const char *bat = find_battery_path();
    if (!bat) return NULL;
...
JNIEXPORT jstring JNICALL
Java_dev_nucleusframework_systeminfo_linux_NativeLinuxSystemInfoBridge_nativeBatteryModelName(
    JNIEnv *env, jclass clazz) {
    const char *bat = find_battery_path();
    if (!bat) return NULL;
...
JNIEXPORT jstring JNICALL
Java_dev_nucleusframework_systeminfo_linux_NativeLinuxSystemInfoBridge_nativeBatterySerialNumber(
    JNIEnv *env, jclass clazz) {
    const char *bat = find_battery_path();
    if (!bat) return NULL;
