...
Java_dev_nucleusframework_systeminfo_linux_NativeLinuxSystemInfoBridge_nativeComponentCount(
...
Java_dev_nucleusframework_systeminfo_linux_NativeLinuxSystemInfoBridge_nativeComponentLabels(
    JNIEnv *env, jclass clazz) {
...
Java_dev_nucleusframework_systeminfo_linux_NativeLinuxSystemInfoBridge_nativeComponentTemperatures(
    JNIEnv *env, jclass clazz) {
    if (g_component_count <= 0) return NULL;
...
JNIEXPORT jfloatArray JNICALL
Java_dev_nucleusframework_systeminfo_linux_NativeLinuxSystemInfoBridge_nativeComponentMaxTemperatures(
    JNIEnv *env, jclass clazz) {
    if (g_component_count <= 0) return NULL;
    jfloat *vals = (jfloat *)malloc(g_component_count * sizeof(jfloat));
...
JNIEXPORT jfloatArray JNICALL
Java_dev_nucleusframework_systeminfo_linux_NativeLinuxSystemInfoBridge_nativeComponentCriticalTemperatures(
    JNIEnv *env, jclass clazz) {
    if (g_component_count <= 0) return NULL;
    jfloat *vals = (jfloat *)malloc(g_component_count * sizeof(jfloat));
