...
Java_dev_nucleusframework_systeminfo_linux_NativeLinuxSystemInfoBridge_nativeGlobalCpuUsage(
    JNIEnv *env, jclass clazz) {
    return (jfloat)compute_global_cpu_usage();
}

JNIEXPORT jint JNICALL
Java_dev_nucleusframework_systeminfo_linux_NativeLinuxSystemInfoBridge_nativePhysicalCoreCount(
    JNIEnv *env, jclass clazz) {
...
JNIEXPORT jint JNICALL
Java_dev_nucleusframework_systeminfo_linux_NativeLinuxSystemInfoBridge_nativeCpuCount(
    JNIEnv *env, jclass clazz) {
    return (jint)count_logical_cpus();
}

JNIEXPORT jobjectArray JNICALL
Java_dev_nucleusframework_systeminfo_linux_NativeLinuxSystemInfoBridge_nativeCpuNames(
    JNIEnv *env, jclass clazz) {
...
JNIEXPORT jobjectArray JNICALL
Java_dev_nucleusframework_systeminfo_linux_NativeLinuxSystemInfoBridge_nativeCpuVendorIds(
    JNIEnv *env, jclass clazz) {
    int count = count_logical_cpus();
    if (count <= 0 || count > MAX_CPUS) return NULL;
...
JNIEXPORT jobjectArray JNICALL
Java_dev_nucleusframework_systeminfo_linux_NativeLinuxSystemInfoBridge_nativeCpuBrands(
    JNIEnv *env, jclass clazz) {
    int count = count_logical_cpus();
    if (count <= 0 || count > MAX_CPUS) return NULL;
    cpuinfo_entry_t *entries = (cpuinfo_entry_t *)calloc(count, sizeof(cpuinfo_entry_t));
...
Java_dev_nucleusframework_systeminfo_linux_NativeLinuxSystemInfoBridge_nativeCpuFrequencies(
    JNIEnv *env, jclass clazz) {
    int count = count_logical_cpus();
    if (count <= 0 || count > MAX_CPUS) return NULL;
    cpuinfo_entry_t *entries = (cpuinfo_entry_t *)calloc(count, sizeof(cpuinfo_entry_t));
...
Java_dev_nucleusframework_systeminfo_linux_NativeLinuxSystemInfoBridge_nativeCpuUsages(
    JNIEnv *env, jclass clazz) {
    int count = count_logical_cpus();
    if (count <= 0 || count > MAX_CPUS) return NULL;
