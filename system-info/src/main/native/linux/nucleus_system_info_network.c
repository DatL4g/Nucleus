...
Java_dev_nucleusframework_systeminfo_linux_NativeLinuxSystemInfoBridge_nativeNetworkCount(
...
Java_dev_nucleusframework_systeminfo_linux_NativeLinuxSystemInfoBridge_nativeNetworkNames(
    JNIEnv *env, jclass clazz) {
...
Java_dev_nucleusframework_systeminfo_linux_NativeLinuxSystemInfoBridge_##jni_name( \
    JNIEnv *env, jclass clazz) { \
    if (g_net_count <= 0) return NULL; \
...
JNIEXPORT jobjectArray JNICALL
Java_dev_nucleusframework_systeminfo_linux_NativeLinuxSystemInfoBridge_nativeNetworkMacAddresses(
    JNIEnv *env, jclass clazz) {
    if (g_net_count <= 0) return NULL;
    const char **arr = (const char **)malloc(g_net_count * sizeof(char *));
