@echo off
REM Compiles nucleus_media_control_windows.cpp into per-architecture DLLs (x64 + ARM64).
...
set "SCRIPT_DIR=%~dp0"
set "SRC=%SCRIPT_DIR%nucleus_media_control_windows.cpp"
...
cl /LD /O1 /EHsc /std:c++17 /GS- /nologo ^
    ...
    "%SRC%" ^
    /Fe:"%OUT_DIR_X64%\nucleus_media_control_windows.dll" ^
    /link ole32.lib runtimeobject.lib kernel32.lib user32.lib
if errorlevel 1 (
...
echo.
echo Built DLLs:
if exist "%OUT_DIR_X64%\nucleus_media_control_windows.dll" echo   %OUT_DIR_X64%\nucleus_media_control_windows.dll
if exist "%OUT_DIR_ARM64%\nucleus_media_control_windows.dll" echo   %OUT_DIR_ARM64%\nucleus_media_control_windows.dll
